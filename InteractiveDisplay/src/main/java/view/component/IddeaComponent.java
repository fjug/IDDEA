...
public class IddeaComponent extends JPanel implements ActionListener, ChangeListener
{
	 ...
	 * 
	 * @param raiSource
	 *            an RandomAccessibleInterval<T> containing the desired view
	 ...
	 * 
	 * @param sourceImage
	 *            an IntervalView<LongType> containing the desired view onto the
	 *            raw image data
	 */
	...
	private void updateView()
	{
		...
		final RealRandomAccessible< DoubleType > interpolated = Views.interpolate( Views.extendZero( interval ), new NearestNeighborInterpolatorFactory< DoubleType >() );
		interactiveViewer2D.updateSource( interpolated );

		// In case of needing to update MinMax values when user changes time or
		// stack

		// final DoubleType min = new DoubleType();
		// final DoubleType max = new DoubleType();
		// ImglibUtil.computeMinMax( interval, min, max );
		// final RealARGBConverter< DoubleType > converter = new
		// RealARGBConverter< DoubleType >( min.get(), max.get() );
		//
		// updateDoubleTypeSourceAndConverter( interpolated, converter );
	}
}
